...
getPCP <- function(nC){
  ...
  plot_clusters = lapply(1:nC, function(i){
    ...
    plotDEG(data = logSoy, dataMetrics = scatMatMetrics, option="scatterPoints", threshVar = "FDR", threshVal = 0.01, degPointColor = colList[i+1], fileName=fileName)
  ...
  plot_filtered = ggplot(boxDat, aes_string(x = 'Sample', y = 'Count')) + geom_boxplot() + geom_line(data=pcpDat, aes_string(x = 'Sample', y = 'Count', group = 'ID'), colour = colList[1], alpha=0.2) + xlab(paste("Filtered (n=", format(nGenes, big.mark=",", scientific=FALSE), ")",sep="")) + ylab("Count")
  ...
  plot_clustersSig = lapply(1:nC, function(i){ 
    ...
    sigID = metricFDR[metricFDR$FDR<0.01,]$ID
