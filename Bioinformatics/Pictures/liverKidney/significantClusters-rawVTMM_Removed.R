...
length(which(sigK_TMM$ID %in% sigK_Raw$ID)) # All 3974 genes in TMM kidney were in raw kidney
length(which(sigK_Raw$ID %in% sigK_TMM$ID)) # 3974 genes in Raw kidney remain in TMM kidney
length(which(!sigK_Raw$ID %in% sigK_TMM$ID)) # 3076 genes in Raw kidney but not in TMM kidney
...
###########################
  ...
  x = as.data.frame(dataqps[which(dataqps$ID %in% removedDEG$ID),])
  x$cluster = "color"
  ...
  pSig = ggplot(boxDat, aes_string(x = 'Sample', y = 'Count')) + geom_boxplot() + geom_line(data=pcpDat, aes_string(x = 'Sample', y = 'Count', group = 'ID'), colour = "darkmagenta", alpha=0.1) + ylab("Standardized Count") + theme(plot.title = element_text(hjust = 0.5, size=18), axis.text=element_text(size=18), axis.title=element_text(size=18)) + ggtitle(paste("Significant Kidney Genes TMM Removed (n=", format(nGenes, big.mark=",", scientific=FALSE), ")",sep=""))
  
  fileName = paste(getwd(), "/", outDir, "/", plotName, "_KSigRemovedWithTMM.jpg", sep="")
  jpeg(fileName)
  plot(pSig)
  invisible(dev.off())
...
getPCP <- function(nC){
  ...
  set.seed(1)
  colList = scales::seq_gradient_pal("orangered4", "darkorange2", "Lab")(seq(0,1,length.out=nC+1))
  k = cutree(hc, k=nC)
  ...
  plot_clusters = lapply(1:nC, function(i){
    ...
    scatMatMetrics[["K_L"]]$ID = as.factor(as.character(scatMatMetrics[["N_P"]]$ID))
    ...
    p = ggplot(boxDat, aes_string(x = 'Sample', y = 'Count')) + geom_boxplot() + geom_line(data=pcpDat, aes_string(x = 'Sample', y = 'Count', group = 'ID'), colour = colList[i+1], alpha=0.5) + ylab("Standardized Count") + ggtitle(paste("Cluster ", i, " Genes (n=", format(nGenes, big.mark=",", scientific=FALSE), ")",sep="")) + theme(plot.title = element_text(hjust = 0.5, size=32), axis.text=element_text(size=32), axis.title=element_text(size=32))
