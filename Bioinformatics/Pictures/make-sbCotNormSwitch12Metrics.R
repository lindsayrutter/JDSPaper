...
# Switch and relabel
data <- data[,c(1,2,3,5,4,6,7,8,9,10)]
...
soybean_cn_switch12_metrics <- list()
...
for (i in 1:(ncol(fit)-1)){
  for (j in (i+1):ncol(fit)){
    ...
    soybean_cn_switch12_metrics[[paste0(colnames(fit)[i], "_", colnames(fit)[j])]] <- lrt
  }
}

save(soybean_cn_switch12_metrics, file = "soybean_cn_switch12_metrics.rda")
