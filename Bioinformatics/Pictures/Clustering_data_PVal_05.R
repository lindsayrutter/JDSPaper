...
plotName = "N_P"
outDir = "Clustering_data_PVal_05"
...
getPCP <- function(nC){
  ...
  for (i in 1:nC){
    ...
    xNames = rownames(x)
    xPValue = metrics[which(metrics$ID %in% xNames),]$PValue
    sbsDF = rbind(sbsDF, data.frame(Cluster = paste("Cluster", i), PValue = xPValue))
  }
  ...
  xPValue = metrics[which(metrics$ID %in% xNames),]$PValue
  sbsDF = rbind(sbsDF, data.frame(Cluster = paste("Filtered"), PValue = xPValue))
  ...
  plot_clustersSig = lapply(1:nC, function(i){ 
    ...
    sigID = metricPValue[metricPValue$PValue<0.05,]$ID
