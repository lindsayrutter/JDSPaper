...
# Changed lower limit of maxRange to be negative
...
withProgress(message = 'Creating plot', value = 0, {
  ...
  n <- 2
    ...
    my_fn <- function(data, mapping, ...){
      ...
      p <- ggplot(hexdf, aes(x=x, y=y, hexID=hexID, counts=counts, fill=countColor2)) + geom_hex(stat="identity") + scale_fill_manual(labels = as.character(my_breaks), values = rev(clrs), name = "Cases count") + geom_abline(intercept = 0, color = "red", size = 0.25) + coord_fixed(xlim = c(maxRange[1]-buffer, maxRange[2]+buffer), ylim = c(maxRange[1]-buffer, maxRange[2]+buffer))
      p
    }
