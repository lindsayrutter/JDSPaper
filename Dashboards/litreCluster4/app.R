...
server <- function(input, output, session) {
  ...
  # updateSelectizeInput(session, "selPair", "Treatment pairs:", choices = myPairs, options = list(maxItems = 2), selected = x)})
  ...
  output$hexPlot <- renderPlotly({
    ...
    p <- reactive(ggplot(hexdf, aes(x=x, y=y, hexID=hexID, counts=counts, fill=countColor2)) + geom_hex(stat="identity") + scale_fill_manual(labels = as.character(my_breaks), values = rev(clrs), name = "Gene count", guide = guide_legend()) + geom_abline(intercept = 0, color = "red", size = 0.25) + labs(x = paste0("Read count ", "(", input$selPair[1], ")"), y = paste0("Read count ", "(", input$selPair[2], ")")) + theme(axis.text=element_text(size=15), axis.title=element_text(size=15), legend.title=element_text(size=15), legend.text=element_text(size=15), legend.position="bottom", legend.direction="horizontal") + coord_fixed(ratio=1))
