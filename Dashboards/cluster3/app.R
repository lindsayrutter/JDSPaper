...
shinydashboard::tabItem(tabName = "about",
    ...
    br(),
    shiny::fluidRow("Go ahead and test this application by switching to the 'Application' tab on the left side of the screen. The data you can interactively examine in that application include the 2009 significant genes (FDR<0.05) from Cluster 3 in the iron metabolism soybean RNA-seq dataset.", style='padding:10px;'),
    br(),
    br(),
...
server <- function(input, output, session) {
  ...
  output$hexPlot <- renderPlotly({
    ...
    p <- reactive(ggplot(hexdf, aes(x=x, y=y, hexID=hexID, counts=counts, fill=countColor2)) + geom_hex(stat="identity") + scale_fill_manual(labels = as.character(my_breaks), values = rev(clrs), name = "Gene count") + geom_abline(intercept = 0, color = "red", size = 0.25) + labs(x = paste0("Read count ", "(", input$selPair[1], ")"), y = paste0("Read count ", "(", input$selPair[2], ")")) + coord_fixed(ratio=1))
