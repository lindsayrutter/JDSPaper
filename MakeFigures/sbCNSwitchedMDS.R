...
#ggplot(plotMDS(data[,2:ncol(data)], col=colors, xlab="Dim 1", ylab="Dim 2", labels=colnames(data[,2:ncol(data)]), xlim=c(-4, 4), ylim=c(-2, 2.5)))

mdsDat = data[,2:ncol(data)]
ggplot(mdsDat, aes(x,y)) + geom_text(data = myDat[c(1:3),], label = rownames(myDat[c(1:3),]), nudge_y = 0.35, fontface="bold", color = "royalblue") + geom_text(data = myDat[c(4:6),], label = rownames(myDat[c(4:6),]), nudge_y = 0.35, fontface="bold", color = "darkorange2") + labs(x = "Dim 1", y = "Dim 2") + theme(text = element_text(size=12))
