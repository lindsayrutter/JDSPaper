...
ret[["K_L"]] + xlab("Logged Count") + ylab("Logged Count") + ggtitle(paste("Cluster ", j, " Significant Added Genes (n=", format(nGenes, big.mark=",", scientific=FALSE), ")",sep="")) + theme(plot.title = element_text(hjust = 0.5, size=11), axis.text=element_text(size=11), axis.title=element_text(size=12), strip.text = element_text(size = 10))
