...
\begin{figure}
\begin{center}
\centerline{\includegraphics[width=14cm]{{/Users/lindz/JDSPaper/Bioinformatics/Pictures/SwitchSample/Switch12/NotSwitch/scatPCP}.png}}
\end{center}
...
\begin{figure}
\begin{center}
\centerline{\includegraphics[width=10cm]{/Users/lindz/JDSPaper/Bioinformatics/Pictures/Figure4.png}}
\end{center}
...
\begin{figure}
\begin{center}
\centerline{\includegraphics[width=12cm]{/Users/lindz/JDSPaper/Bioinformatics/Pictures/mdsSwitch.png}}
\end{center}
\caption{Traditional plots showing the cotyledon data before sample switching (left half) and after sample switching (right half). We cannot detect from the boxplots that samples S1.3 and S2.1 may have been swapped (subplots A). This is because all six samples have similar five number summaries. The MDS plot on the left does show a cleaner separation between the two treatment groups in the first dimension than it does on the right (subplots B). However, the second dimension creates three clusterings of the samples, with sample S1.1 appearing as an outlier. As a result, collectivly, it is difficult to suspect that these two samples may have been swapped by examining the MDS plots (subplots B). This is because we cannot obtain information about the variation at the gene-level through the MDS plots like we can through the scatterplot matrix.
\label{mdsSwitch}}
\end{figure}
